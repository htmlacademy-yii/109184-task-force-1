...
<div class="main-container page-container">
  <section class="menu-toggle">
    <ul class="menu-toggle__list">
      <li class="menu-toggle__item menu-toggle__item--completed <?= (isset($filter['status']) && $filter['status'] == 'finished') ? 'menu_toggle__item--current' : '' ?>">
        ...
        <a href="<?= Url::to(['/mytasks', 'status' => 'finished']);?>">
          Завершённые
        </a>
      </li>
      <li class="menu-toggle__item  menu-toggle__item--new <?= ((isset($filter['status']) && $filter['status'] == 'new') || !isset($filter['status'])) ? 'menu_toggle__item--current' : '' ?>">
        ...
        <a href="<?= Url::to(['/mytasks', 'status' => 'new']);?>">
          Новые
        </a>
      </li>
      <li class="menu-toggle__item menu-toggle__item--current <?= (isset($filter['status']) && $filter['status'] == 'current') ? 'menu_toggle__item--current' : '' ?> ">
        <div class="menu-toggle__svg-wrapper">
        ...
        <a href="<?= Url::to(['/mytasks', 'status' => 'current']);?>">
          Активные
        </a>
      </li>
      <li class="menu-toggle__item menu-toggle__item--canceled <?= (isset($filter['status']) && $filter['status'] == 'canceled') ? 'menu_toggle__item--current' : '' ?>">
        <div class="menu-toggle__svg-wrapper">
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path fill-rule="evenodd" clip-rule="evenodd"
        ...
        <a href="<?= Url::to(['/mytasks', 'status' => 'canceled']);?>">
          Отменённые
        </a>
      </li>
      <li class="menu-toggle__item menu-toggle__item--hidden <?= (isset($filter['status']) && $filter['status'] == 'expired') ? 'menu_toggle__item--current' : '' ?>">
        <div class="menu-toggle__svg-wrapper">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <a href="<?= Url::to(['/mytasks', 'status' => 'expired']);?>">
          Просроченные
        </a>
      </li>
    </ul>
